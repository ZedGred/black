...
@section('dashboard-content')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            const toggleLike = (btn, type, id) => {
                ...
                btn.innerText =
                    `${!isLiked ? 'Liked' : 'Like'} (${!isLiked ? currentCount + 1 : currentCount - 1})`;
                ...
                axios({
                    ...
                    .catch(err => {
                        if (err.response && err.response.status === 409) {
                            ...
                            if (!
                                isLiked
                            ) { // sebelumnya kita kirim post, server bilang 409 → berarti harus unlike
                                axios.delete(url, {
                                        withCredentials: true,
                                        headers: {
                                            'Accept': 'application/json',
    ...
    ${isauth == 1 ? `
        <button id="btnLikeArticle-${article.id}" 
                class="btn btn-sm d-flex align-items-center ${isLiked ? 'btn-secondary' : 'btn-outline-primary'}" 
                data-liked="${isLiked}" 
                data-id="${article.id}">
    ...
    <button id="btnCommentArticle-${article.id}" 
        ...
        `;
                ...
                if (btnLikeArticle) {
                    btnLikeArticle.addEventListener('click', () => toggleLike(btnLikeArticle, 'article', article
                        .id));
                }
            };
    ...
    <button id="btnCommentArticle-${c.id}" 
            class="btn btn-sm btn-outline-secondary d-flex align-items-center" 
            data-id="${c.id}">
        <i class="bi bi-chat-left-text me-1 fs-5"></i>
            ...
            const loadArticle = () => {
                axios.get(`/api/articles/${articleId}`, {
                    ...
                    .catch(err => {
                        console.error(err);
                        container.innerHTML =
                            '<p class="text-danger text-center">Failed to load article</p>';
                    });
            };
    ...
    <button id="btnCommentArticle-${c.id}" 
        ...
        <span class="fw-semibold fs-6">${c.comments_count || 0}</span>
            ...
            `;
                            ...
                            const newBtn = commentsList.querySelector(
                                `.btn-like-comment[data-comment-id="${c.id}"]`);
